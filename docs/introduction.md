...
* **Your storage, your account, your rates.** Vaults live in S3 buckets *you* own — AWS S3, MinIO, or any
  [S3-compatible provider](https://docs.cyberduck.io/protocols/s3/). You pay the storage provider
...
* **Access control that follows membership.** With _Scoped Credentials_, vault membership is mirrored to Keycloak and clients
  exchange their OIDC token ([RFC 8693](https://www.rfc-editor.org/rfc/rfc8693.html)) for short-lived S3 credentials scoped to a
  single vault's S3 bucket (AWS STS or MinIO STS; AWS additionally uses role chaining). No component holds standing storage
...
* _Storage Access_: Vault membership defines storage access managed by Katta Server:
    - **Static Credentials**: the access key is shared among vault members in an end-to-end encrypted way with zero trust in Katta Server
    - **Scoped Credentials**: vault membership is mirrored in Keycloak, and the access tokens issued by Keycloak are evaluated by STS for fine-grained storage access control to a single S3 bucket.
