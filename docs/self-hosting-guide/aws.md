...
* reads the Keycloak realm URL and client IDs from the public configuration of Katta Server at `<hub-url>/api/config`;
* registers the Keycloak realm as an OpenID Connect identity provider in AWS IAM for the `cryptomator`, `cryptomatorhub`
  and `cryptomatorvaults` clients;
...
:::tip
It is idempotent — re-run it to pick up policy changes. Renewed Keycloak TLS certificates need no re-run, see [Identity Provider](#identity-provider).
:::
...
export HUB_URL=[your Katta Server URL, e.g. https://hub.example.com]
katta setup aws --hubUrl "${HUB_URL}"
```

:::tip
The `katta` command prints the Keycloak realm URL and client IDs it uses, and the equivalent `aws iam …` for the role and policy API calls it performs.
:::
...
| Provider URL | `<keycloakUrl>/realms/<keycloakRealm>` from `<hub-url>/api/config`                              |
| Audiences    | `cryptomator` (Katta Desktop), `cryptomatorhub` (Katta Web), `cryptomatorvaults` (Vault Access) |

The audiences are the client IDs `keycloakClientIdCryptomator`, `keycloakClientIdHub` and `keycloakClientIdCryptomatorVaults`
from `<hub-url>/api/config`.

No certificate thumbprint is set. AWS verifies the TLS certificate of the Keycloak endpoint against its
[library of trusted root certificate authorities](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_providers_create_oidc_verify-thumbprint.html),
so renewed certificates need no update. If Keycloak uses a certificate not signed by one of these CAs, add its thumbprint to the
identity provider in IAM manually.

If a provider for the same URL already exists and its audience list covers all client IDs, it is left unchanged. If the audience
list differs, the provider is deleted and recreated with the full list. The ARN is derived from the provider URL and therefore
stays the same across recreation.
